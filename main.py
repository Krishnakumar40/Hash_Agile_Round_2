...
# Connect to Elasticsearch with increased timeout and ignore SSL verification (if necessary)
es = Elasticsearch("http://localhost:8989", timeout=60, verify_certs=False)

# Check if Elasticsearch is running and respond accordingly
try:
    if es.ping():
...
# Create dynamic collection names
...
def indexData(p_collection_name, p_exclude_column):
    try:
        data = pd.read_csv("employee_data.csv", encoding='ISO-8859-1').drop(columns=[p_exclude_column])
        actions = [
            {
...
# Function to search data by column in the collection
...
# Function to get employee count from the collection
...
# Function to delete an employee by ID from the collection
...
# Function to get department-wise employee count
...
# Create collections for both name and phone
createCollection(v_nameCollection)
createCollection(v_phoneCollection)

# Get the initial employee counts for each collection
getEmpCount(v_nameCollection)

# Index data into both collections (excluding columns)
...
# Delete an employee by ID and check the updated count
